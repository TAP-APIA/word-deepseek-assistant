...
> 提示：在输入框按 **回车** 直接发送，**Shift+回车** 换行。

## 一键安装脚本（推荐）

如果不想手动配置"受信任的加载项目录"，项目里提供了两个脚本：

- 双击 **`install.bat`**：自动请求管理员权限，依次完成：把 `manifest.xml` 复制到 `C:\WordAddins` → 创建网络共享 `WordAddins` → 注册受信任目录（注册表）→ 最终校验。之后只需重启 Word，在 **插入 → 加载项 → 我的加载项 → 共享文件夹** 里添加插件。
- 双击 **`uninstall.bat`**：移除网络共享、注册表登记，并可选择删除 `C:\WordAddins` 文件夹。

说明：

- 脚本优先使用同目录下的 `manifest.xml`；如果没有，会自动从 GitHub 下载最新版。
- 需要管理员权限（双击脚本会弹出 UAC 授权窗口，点是即可）。
- 在其他电脑上使用：把 `install.bat`、`install.ps1`、`manifest.xml` 三个文件放到同一文件夹，双击 `install.bat` 即可。
